...
async def grant_role(ctx, roleType, user: discord.User):
    logger.info(f'!role command triggered by user {ctx.author.name} ({ctx.author.id}) for {user.display_name} ({user.id}) to change {roleType} tags')
    if None != ctx.guild.get_member(user.id).get_role(enums.GRANTABLE_ROLES["regiment"]):
        ...
        if None != ctx.guild.get_member(user.id).get_role(roleToManage):
            logger.info(f' {user.display_name}({user.id}) already has {roleType} tags. Will remove them now...')
            try:
