...
async def grant_role(ctx, roleType, user: discord.User, log):
    log.info(f'!role command triggered by user {ctx.author.name} ({ctx.author.id}) for {user.display_name} ({user.id}) to change {roleType} tags. Checking status of user...')
    if None != ctx.guild.get_member(user.id).get_role(enums.GRANTABLE_ROLES["regiment"]):
        log.info(f'Error running !role command - user {user.display_name} ({user.id}) is already enlisted in the 87th.')
